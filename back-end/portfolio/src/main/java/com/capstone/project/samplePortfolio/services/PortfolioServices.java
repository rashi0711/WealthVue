...
public class PortfolioServices {
    ...
    public List<PortfolioResponse> getAllPortfolio(String userId) {
        ...
        List<Portfolio> userPortfolio = portfolioList.stream().filter(portfolio -> portfolio.getUserID().equals(userId)).collect(Collectors.toList());
        ...
        if(portfolio.isPresent()){
            ...
            if(stock!=null){
                ...
                if(previousStock.isEmpty()){
                    ...
                    Stock newStock=new Stock(stockId,stock.getName(),stock.getImage(),stock.getSymbol(),numberOfStocks,stock.getCurrentPrice()*numberOfStocks,0.0,stock.getCurrentPrice(),transactionsList);
                    stocksList.add(newStock);
                }else{
                    double totalBuy=previousStock.get().getTotalBuy()+(stock.getCurrentPrice()*numberOfStocks);
    ...
    public List<PortfolioData> getPortfolioData(String userId) {
        ...
        getThreePortfolios.forEach(p->{
                ...
                try {
                    ...
                    PortfolioData portfolioData=new PortfolioData(p.getPortfolioName(),stockdetailsData.getTotalBalance(),stockdetailsData.getTotalReturns());
